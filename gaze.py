...
def calculate_angle(x1, y1, x2, y2):
    # Calculate the angle in radians between the line and x-axis
...
def gaze(frame, points):
    ...
    if transformation is not None:  # if estimateAffine3D secsseded
        ...
        slope = calculate_angle(p1[0], p1[1], p2[0], p2[1])
        slope = round(slope, 2)
        cv2.putText(frame, f"GazeSlope: {slope}", (30, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 0) , 2, cv2.LINE_AA) 
        return slope
